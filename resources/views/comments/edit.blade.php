        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <textarea
                    ...
                    name="text"
                    id="text"
                    >{{old('text', $comment->text)}}</textarea>
                @error('text')
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <select
                    ...
                    <option value="{{ $user->id }}" @if(old('user_id', $comment->user_id) == $user->id) selected @endif >{{ $user->first_name }} {{ $user->last_name }} - {{ $user->username }}</option>
                    @endforeach
                </select>
                @error('user_id')
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10">
                <select
                    ...
                    <option value="{{ $task->id }}" @if(old('task_id', $comment->task_id) == $task->id) selected @endif >{{ $task->name }} - {{ $task->status }}</option>
                    @endforeach
                </select>
                @error('task_id')
